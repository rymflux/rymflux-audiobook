	...
	<div class="flex-1 min-w-0">
		...
		{#if savedProgress > 0}
			<div class="mt-3 space-y-1">
				<p class="text-xs text-gray-500">Progress: {Math.round((savedProgress / ((book.item.total_time_secs ?? 1) * 1000)) * 100)}%</p>
	...
	<section>
		...
		<div class="space-y-1">
			{#each book.sections as chapter, i (chapter.id)}
				...
				>
					<span class="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-xs font-medium shrink-0">
						{i + 1}
					</span>
					<div class="flex-1 min-w-0">
					...
					{#if chapter.playtime_secs != null}
						<span class="text-xs text-gray-500 shrink-0"><TimeDisplay seconds={chapter.playtime_secs} /></span>
					{/if}
				</button>
			{/each}
		</div>
